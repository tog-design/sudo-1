...
function start() {
  ...
  console.assert(calculo(3) === undefined, '1');
  console.assert(calculo(8, 12) === 4, '2');
  console.assert(calculo(9, 27) === 9, '3');
  console.assert(calculo(259, 111) === 37, '4');
}

function calculo(a, b) {
  if ( !a || !b ) { 
      return; // Quando não tiver duas pilhas
  }

  const maior = Math.max(a, b);
  const menor = Math.min(a, b);

  let menorRest = Math.min(a, b);
  while ( maior % menorRest !== 0 || menor % menorRest !== 0) {
      menorRest--;
  }
